...
// Modified for Jumps Mod by PlusIce
// Modified by Kraken - added debug logging
//***********************************************************************************************************

Initialise()
{
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Initialise has begun.");
		wait 1;
	}

	if(!isDefined(level.awe_mapvote) || !level.awe_mapvote) return;
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: 2.");
		wait 1;
	}

	// Since we're not running AWE but pretending to we need to define some stuff first.
	...
	level.mapvotetext["MapVoteHeader"] = &"Next Map Vote";
		//if (isDefined(level.lev_debug) && level.lev_debug) iprintln("starting precache");
	...
	//if (isDefined(level.lev_debug) && level.lev_debug) iprintln("checkpoint002");
	...
	//if (isDefined(level.lev_debug) && level.lev_debug) iprintln("checkpoint003");
	...
	//if (isDefined(level.lev_debug) && level.lev_debug) iprintln("checkpoint004");
	precacheShader("white");
	//if (isDefined(level.lev_debug) && level.lev_debug) iprintln("checkpoint005");
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Finished precaching.");
		wait 2;
	}

	if (isDefined(level.lev_debug) && level.lev_debug)
	{
		iprintln("Does this iprintln call work?");
		wait 1;
	}



	// Precaching map vote strings
	//PrecacheMapVoteStrings();
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Finished CleanUp().");
		wait 2;
	}
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Finished creating HUD.");
		wait 3;
	}
...
CreateHud()
{
	if (!isDefined(level.awe_mapvotehudoffset))
	{
		if (isDefined(level.lev_debug) && level.lev_debug) {
			iprintln("^3DEBUG: awe_mapvotehudoffset was not defined, setting now.");
			wait 1;
		}	
		level.awe_mapvotehudoffset = 30;
	} 
	if (!isDefined(level.mapvotetext)) level.mapvotetext = 0;
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Finished through bgnd section.");
		wait 1;
	}	
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Got through header section.");
		wait 1;
	}	
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Got through header text section.");
		wait 1;
	}	
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Got through leftline section.");
		wait 1;
	}	
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Got through rightline section.");
		wait 1;
	}	
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Got through bottomline section.");
		wait 1;
	}	
...
/*	level.vote_hud_votestext = newHudElem();
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Got through timeleft section.");
		wait 1;
	}	
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Got through instructions section.");
		wait 1;
	}	
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Got through maps sections.");
		wait 1;
	}
}
...
{
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Starting RunMapVote() thread.");
		wait 3;
	}
	...
	x = maps\mp\gametypes\_lev::GetRandomMapRotation();
	if(isdefined(x))
	{
		if(isdefined(x.maps))
			maps = x.maps;
	...
	if(!isdefined(maps))
	{
		...
		if (isDefined(level.lev_debug) && level.lev_debug)
		{
			iprintln("^1DEBUG: No maps found, can't do vote");
			wait 5;
		} 
	...
	for(j=0;j<5;j++)
	{
		...
		level.mapcandidate[j]["mapname"] = maps\mp\gametypes\_lev::getMapName(maps[i]["map"]);
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Finished getting candidates.");
		wait 3;
	}
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Finished displaying map choices.");
		wait 3;
	}
	...
	if (isDefined(level.lev_debug) && level.lev_debug) {
		iprintln("^2DEBUG: Finished play threads and vote logic.");
		wait 3;
	}
...
PlayerVote()
{
	...
	self maps\mp\gametypes\_lev::spawnSpectator();
	...
	if (!isDefined(level.awe_mapvotehudoffset)) level.awe_mapvotehudoffset = 30;
...
SetMapWinner(winner)
{
	...
	iprintlnbold("^2" + maps\mp\gametypes\_lev::getGametypeName(gametype));
