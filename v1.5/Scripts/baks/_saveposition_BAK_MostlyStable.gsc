	...
	self endon("end_saveposition_threads");
{
		...
		self iprintln("double-click^5 [{+melee}] ^7: to save position");
wait 3;
		self iprintln("double-click^1 [{+activate}] ^7: to load saved position");
	}
}
_MeleeKey()
...
_UseKey()
{
	...
	for(;;)
	{
		...
		{
			iprintln("DEBUG: useButtonPressed once");
			...
			for(i=0; i<=0.30; i+=0.01)
			{
				...
				{
					iprintln("DEBUG: catchNext and useButtonPressed");
...
savePos()
{
	...
	self iprintln("^2P^7osition ^3(^5" + (int)self.saved_origin[0] + "^7, ^5" + (int)self.saved_origin[1] + "^7, ^5" + (int)self.saved_origin[2] + "^3)^7 saved.");
}
...
loadPos()
{
	iprintln("DEBUG: starting loadPos");
	...
	if(!isDefined(self.saved_origin))
		...
		self iprintln("^2P^7revious position ^3(^1" + (int)self.saved_origin[0] + "^7, ^1" + (int)self.saved_origin[1] + "^7, ^1" + (int)self.saved_origin[2] + "^3)^7 loaded.");
		}
}

loadPossave()
{
	iprintln("DEBUG: starting loadPossave");
	...
	{
		iprintln("DEBUG: trying to load prev position");
		...
		{
			iprintln("DEBUG: clear for landing");
			self setPlayerAngles(self.saved_angles); // angles need to come first
			self setOrigin(self.saved_origin);
			self iprintln("^2P^7revious position ^3(^1" + (int)self.saved_origin[0] + "^7, ^1" + (int)self.saved_origin[1] + "^7, ^1" + (int)self.saved_origin[2] + "^3)^7 loaded.");
		}
